...
let union (x : 'a pointRef) (y : 'a pointRef) : 'a pointRef =
    ...
    if rx == ry then
        ...
        match !rx, !ry with
        | Root ({ rank = rankx; _} as nx),
          Root { rank = ranky; _} ->
            if rankx > ranky then begin
                ry := Link { parent = rx };
                rx
            end else if rankx < ranky then begin
                rx := Link { parent = ry };
                ry
            end else begin
                ry := Link { parent = rx };
...
let unionWith (f : 'a -> 'a -> 'a) (x : 'a pointRef) (y : 'a pointRef) : 'a pointRef = 
    let (rx, ry) = (find x, find y) in
    if rx == ry then
        rx
    else
